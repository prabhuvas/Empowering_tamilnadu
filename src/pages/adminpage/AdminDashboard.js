...
import GeneralUnresolved from "./GeneralUnresolved";
import { useTranslation } from "react-i18next";
import { DocumentCheckIcon, DocumentIcon } from "@heroicons/react/24/outline";
import hero from "../../images/generalcomplaint.png";
...
const AdminDashboard = () => {
  ...
  return (
    <div style={{ backgroundColor: "#fffde8" }}>
      <div>
        <AdminContent color={true} />
      </div>
      <br />
      <br />

      {/* grid grid-cols-1 sm:mx-auto sm:w-full sm:max-w-sm lg:max-w-7xl md:grid-cols-1 */}
          ...
          {unResolvedComp &&
            complaint?.map((post) => {
              if (
                ...
                post.urgentComplaint === false
              ) {
                if (post === null) {
                  return <div>No Complaints</div>;
                } else {
                  return <GeneralUnresolved post={post} />;
                }
              } else if (
          ...
          {resolvedComp &&
            complaint?.map((post) => {
              if (
                ...
                return <AdminComplaints post={post} />;
              } else if (
                email === "maduraiwater@gmail.com" &&
                post.type === "Water" &&
                post.department === "Madurai" &&
