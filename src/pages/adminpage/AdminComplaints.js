...
const AdminComplaints = (props) => {
  ...
  const setStatus = async (id) => {
    if (state) {
      ...
      const anothernewField = { status: "ongoing" };
  ...
  return (
    <div
      className="flex px-6 py-5 lg:px-8 text-left xs:bg-white"
      style={{ backgroundColor: "#fffde8", borderRadius: "15px" }}
    >
      {post?.status === "Issue-Rectified" && (
        <div
          className="bg-white rounded-lg shadow-lg p-6 mt-5 sm:mx-auto sm:w-full sm:max-w-sm"
          style={{ border: "1px solid orange" }}
        >
          {/* <div
            className="pb-5"
            style={{ display: details ? "none" : "", float: "right" }}
          >
            <button
              onClick={() => {
                handleDetails(post?.id, post?.status);
              }}
            >
              <PlusCircleIcon className="h-6 w-6 mt-3" />
            </button>
          </div>
          ...
          <div
            ...
            >
              ...
              >
                <XCircleIcon className="h-5 w-5 mt-3" />
              </button>
            </div> */}
